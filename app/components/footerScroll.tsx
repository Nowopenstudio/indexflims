...
export default function FooterScroll({ data, time }: any) {
  ...
  return (
    <motion.div
      ref={ref}
      className="flex left-0 items-center top-[0] z-[1] gap-[200px] w-max h-full navHold"
    ...
    >
      {Array.from({ length: ITEM_COUNT }).map((_, i) => (
        ...
        >
          ...
          <Logo className="w-[1500px] h-auto fadeIn" fill="white" style={{ animationDelay: `${(i * .15)}s` }} />
        </Link>
      ))}
